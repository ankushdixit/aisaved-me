...
describe("Japanese Theme Pagination Component", () => {
  const defaultProps = {
    currentPage: 1,
    totalPages: 5,
  ...
  describe("Japanese Theme Styling", () => {
    it("applies Japanese theme border styling to buttons", () => {
      render(<Pagination {...defaultProps} />);
      const prevButton = screen.getByLabelText("Previous page");
      expect(prevButton).toHaveClass("border", "border-[#d4d0c8]");
    });

    it("current page has Japanese dark background", () => {
      render(<Pagination {...defaultProps} currentPage={2} />);
      const currentPageButton = screen.getByLabelText("Page 2");
      expect(currentPageButton).toHaveClass("bg-[#1a1a1a]", "text-[#faf8f5]");
    });

    it("inactive pages have Japanese light border", () => {
      render(<Pagination {...defaultProps} currentPage={1} />);
      const inactivePageButton = screen.getByLabelText("Page 2");
      expect(inactivePageButton).toHaveClass("border-[#d4d0c8]", "text-[#1a1a1a]");
    });

    it("applies Japanese hover border to buttons", () => {
      render(<Pagination {...defaultProps} />);
      const pageButton = screen.getByLabelText("Page 2");
      expect(pageButton).toHaveClass("hover:border-[#1a1a1a]");
    });

    it("buttons are compact and minimalist", () => {
      render(<Pagination {...defaultProps} />);
      const prevButton = screen.getByLabelText("Previous page");
      expect(prevButton).toHaveClass("w-9", "h-9");
    });

    it("uses Japanese theme gray for ellipsis", () => {
      render(<Pagination {...defaultProps} totalPages={15} currentPage={8} />);
      const ellipsis = screen.getAllByText("...")[0];
      expect(ellipsis).toHaveClass("text-[#6b6b6b]");
    });
  });
  ...
  describe("Interactive Behavior", () => {
    ...
    it("applies hover border effect", () => {
      render(<Pagination {...defaultProps} />);
      const prevButton = screen.getByLabelText("Previous page");
      expect(prevButton).toHaveClass("hover:border-[#1a1a1a]");
    });
  ...
  describe("Layout and Spacing", () => {
    it("uses Japanese theme minimal gap between elements", () => {
      render(<Pagination {...defaultProps} />);
      const nav = screen.getByRole("navigation");
