...
const singleArtifact: StoryArtifact[] = [
  {
    id: "artifact-single",
    type: "image",
    url: "/placeholders/single-evidence.jpg",
...
describe("StoryArtifacts Component", () => {
  ...
  it("renders image type artifacts with correct icon", () => {
    ...
    // Check that the image icon (📷 - camera emoji) is rendered
  ...
  it("renders document type artifacts with correct icon", () => {
    ...
    // Check that the document icon (📄 - page facing up emoji) is rendered
    const icon = container.querySelector("span.text-lg");
    expect(icon).toBeInTheDocument();
    expect(icon?.textContent).toBe("📄");
  });
  ...
  it("renders screenshot type artifacts with correct icon", () => {
    ...
    // Check that the screenshot icon (💻 - laptop emoji) is rendered
    const icon = container.querySelector("span.text-lg");
    expect(icon).toBeInTheDocument();
    expect(icon?.textContent).toBe("💻");
  });
  ...
  it("applies Organic design styling to artifact cards", () => {
    const { container } = render(<StoryArtifacts artifacts={singleArtifact} />);

    // Check for Organic-specific classes (now on button elements)
    const artifactCard = container.querySelector("button.bg-white.rounded-xl.shadow-soft");
    expect(artifactCard).toBeInTheDocument();
  });

  it("makes artifact cards clickable", () => {
    const { container } = render(<StoryArtifacts artifacts={singleArtifact} />);

    // Check that artifacts are rendered as buttons
